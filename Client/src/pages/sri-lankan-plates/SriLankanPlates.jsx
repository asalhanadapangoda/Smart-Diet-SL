...
const SriLankanPlates = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 relative">
      <h1 className="text-4xl md:text-5xl font-bold mb-4 text-gray-800 text-glass text-center">
        {t('generatePlate')}
      </h1>
      <p className="text-gray-700 mb-6 text-glass text-center text-lg">
      ...
      <div className="glass-card rounded-2xl p-6 mb-6 backdrop-blur-xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            >
              {goals.map((g) => (
                <option key={g.value} value={g.value} className="bg-white">
                  {t(g.labelKey)}
                </option>
              ))}
            </select>
          ...
          <div className="flex items-end">
            <label className="flex items-center cursor-pointer">
              ...
              />
              <span className="text-sm text-gray-800 text-glass">{t('busyLifeHack')}</span>
            </label>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="glass-button mt-4 w-full md:w-auto px-6 py-3 text-white rounded-xl hover:scale-105 transition-all disabled:opacity-50 font-medium bg-green-600"
        >
      ...
      {plate && (
        <div className="glass-card rounded-2xl p-6 backdrop-blur-xl">
          ...
          {plate.displayDescription && (
            <p className="text-gray-600 mb-4 text-glass">{plate.displayDescription}</p>
          )}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Plate Items */}
            <div>
              <h3 className="font-semibold mb-3 text-gray-800 text-glass">{t('plateContents')}:</h3>
            ...
            {/* Nutrition Summary */}
            <div>
              <h3 className="font-semibold mb-3 text-gray-800 text-glass">{t('nutritionSummary')}:</h3>
              <div className="glass-card bg-green-100 p-4 rounded-xl space-y-2 border border-green-300">
                <div className="flex justify-between">
                  <span className="text-gray-700 text-glass">{t('totalCalories')}:</span>
                  <span className="font-semibold text-green-700 text-glass">
                    {Math.round(plate.totalNutrition?.calories || 0)} cal
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 text-glass">{t('protein')}:</span>
                  <span className="text-gray-800 text-glass">{Math.round(plate.totalNutrition?.protein || 0)}g</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 text-glass">{t('carbs')}:</span>
                  <span className="text-gray-800 text-glass">{Math.round(plate.totalNutrition?.carbs || 0)}g</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 text-glass">{t('fat')}:</span>
                  <span className="text-gray-800 text-glass">{Math.round(plate.totalNutrition?.fat || 0)}g</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 text-glass">{t('fiber')}:</span>
                  <span className="text-gray-800 text-glass">{Math.round(plate.totalNutrition?.fiber || 0)}g</span>
                </div>
              </div>

              {plate.substitutions && plate.substitutions.length > 0 && (
                <div className="mt-4">
                  <h4 className="font-semibold mb-2 text-gray-800 text-glass">{t('suggestedSubstitutions')}:</h4>
                  <ul className="space-y-1 text-sm">
                    {plate.substitutions.map((sub, index) => (
                      <li key={index} className="text-gray-600 text-glass">
              ...
              {plate.prepTime > 0 && (
                <div className="mt-4 text-sm text-gray-600 text-glass">
